...
In Polars, the `SQLContext` provides a way to execute SQL statements against LazyFrames using SQL syntax. One of the SQL statements that can be executed using `SQLContext` is the `CREATE TABLE` statement, which is used to create a new table.
...
{{code_block('user-guide/sql/create','create',['SQLregister','SQLquery'])}}
...
In this example, we use the `query()` method of the `SQLContext` to execute a `CREATE TABLE` statement that creates a new table called `older_people` based on a SELECT statement that selects all rows from the `my_table` DataFrame where the `age` column is greater than 30.
