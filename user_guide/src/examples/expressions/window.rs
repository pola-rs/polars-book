...
let out = df.clone().lazy().select(
    [
        col("Type 1"),
        col("Type 2"),
        col("Attack").mean().over([col("Type 1")]).alias("avg_attack_by_type"),
        col("Defense").mean().over(["Type 1", "Type 2"]).alias("avg_defense_by_type_combination"),
...
let out = filtered.lazy().with_columns(
    [
        cols(["Name", "Speed"]).sort(true).over([col("Type 1")])
    ]    
).collect()?;
println!("{:?}", out);
...
let out = df.clone().lazy().select(
    [
        col("Type 1").head(Some(3)).list().over([col("Type 1")]).flatten(),
        col("Name").sort_by([col("Speed")], [false]).head(Some(3)).list().over([col("Type 1")]).flatten().alias("fastest/group"),
        col("Name").sort_by([col("Attack")], [false]).head(Some(3)).list().over([col("Type 1")]).flatten().alias("strongest/group"),
        col("Name").sort(false).head(Some(3)).list().over([col("Type 1")]).flatten().alias("sorted_by_alphabet"),
    ]
).collect()?;
println!("{:?}", out);
...
// output type: -> i32
(col("x").sum() * col("y")).over([col("groups")]).alias("x1"),

// sum within a group and multiply with group elements 
// and aggregate the group to a list
// output type: -> ChunkedArray<i32>
(col("x").sum() * col("y")).list().over([col("groups")]).alias("x2"),
...
(col("x").sum() * col("y")).list().over([col("groups")]).flatten().alias("x3"),
]).collect()?;
