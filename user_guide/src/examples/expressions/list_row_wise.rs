...
// ANCHOR: row-wise
let out = grades.clone().lazy().select([
...
let rank_opts = RankOptions { method: RankMethod::Average, descending: true };
let rank_pct = col("").rank(rank_opts) / col("").count().cast(DataType::Float32);

let grades = grades.clone().lazy().with_column(
    ...
    col("all_grades").arr().eval(rank_pct, true).alias("grades_rank")
]).collect()?;
println!("{}", grades);
