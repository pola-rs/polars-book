...
if __name__ == "__main__":
    joined.show_graph(
        optimized=False,
        show=False,
        output_path="user_guide/src/img/projection_pushdown_0.png",
    )
    joined.show_graph(
        optimized=True,
        show=False,
        output_path="user_guide/src/img/projection_pushdown_0_optimized.png",
    )
    df = joined.fetch(int(1e7))
    with open("user_guide/src/outputs/projection_pushdown_0.txt", "w") as f:
        f.write(str(df))
